...
**Recommendation**: keep Lottie, but *replace* `splash.json` with a **2-stage choreography**: (1) brand orb forms from sage→orange particles, (2) orb cracks open into the CallVault wordmark which then anchors at the top of the next screen via shared-element transition.

Rationale (3 lines): (1) Lottie infra is already in place — zero new deps, zero new build-config. (2) A particle-form-into-mark conveys "we capture and consolidate" — the app's actual job. (3) Shared-element handoff to the Welcome screen makes the splash feel like a *first frame*, not a delay.
...
| 1800 | Orb "cracks": top half lifts as a subtle wordmark "CallVault" in `displaySmall` italic; orb dissolves; transition begins. |
...
4. Subtext (sans, 85% white): **"Calls land in your call log. CallVault catches them, saves them, and turns them into leads — automatically."**
...
|---|---|---|---|---|
| 1 | Capture | Lottie: phone ringing → log entry materializes | "Nothing slips through." | "Every incoming, outgoing, and missed call is logged the moment it ends — even when CallVault isn't open." |
...
**Recommendation**: keep the existing `PermissionsPage.kt` *position* in the pager (page 3) but redesign as **one concise screen** with **grouped reasoning**, not 4 equal rows.

Layout:
1. Headline (serif italic): **"Three things we need."** (4th — Notifications — framed as optional below).
2. Sub: **"CallVault works on-device. Nothing leaves your phone."**
...
Microcopy principle: lead with what we *do*, not what we *want*. The line "Nothing leaves your phone" is the trust anchor.
...
3. Tighten body to two short paragraphs:
   - "**You said no, and we respect that.** But CallVault can't read your calls without permission."
...
```xml
<!-- Splash -->
<string name="cv_splash_brand">CallVault</string>

<!-- Welcome -->
<string name="onboarding_welcome_headline">Every inquiry, captured.</string>
<string name="onboarding_welcome_subtext">Calls land in your call log. CallVault catches them, saves them, and turns them into leads — automatically.</string>
...
<string name="onboarding_capture_body">Every incoming, outgoing, and missed call is logged the moment it ends — even when CallVault isn\'t open.</string>
...
<string name="onboarding_permissions_v2_subtitle">CallVault works on-device. Nothing leaves your phone.</string>
...
<string name="permission_denied_v2_body_1">You said no, and we respect that. But CallVault can\'t read your calls without permission.</string>
