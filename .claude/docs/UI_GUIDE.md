...
What an agent needs to build a CallVault screen without exploring. Last refreshed 2026-05-06.

## Design system at a glance

CallVault uses a **custom neumorphic system** ("Neo*"). Two kinds of surfaces — convex (raised) and concave (pressed-in) — render as the same base color `NeoColors.Base` with light/dark soft-shadow pairs from `NeoShadows.kt`. Material 3 typography is reused; Material widgets (Button, Card, TextField) are **not** for production surfaces — use Neo* equivalents.
