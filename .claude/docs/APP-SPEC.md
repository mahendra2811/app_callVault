...
This is the canonical product/UI specification for CallVault. It is intended for any designer or engineer rebuilding the UI from scratch in any framework. Reading this file alone, you should be able to mock every screen accurately and understand every input, output, edge case, and copy line.
...
**Source documents**: `CLAUDE.md`, `.claude/docs/06-glossary.md`, `CHANGELOG.md`, `DECISIONS.md`, plus the locked spec at `~/Documents/p_projet/a_APP/callvault_mega_prompt.md` (1533 lines, design intent unchanged).
...
# CallVault APP-SPEC — Part 03

## Main scaffold + 4 tab pages

> Audience: a UX engineer rebuilding the CallVault Android UI from scratch.
...
| Deep-link `callvault://calls` | Routes through rootNav → MainScaffold → mainTabNav.navigate("calls").                     |
...
- `showBrand = true` → leading slot renders the small CallVault wordmark
  (`logoMonochrome` painter at 18.sp height).
- `title = "callVault"` (lowercase brand). Style: `MaterialTheme.typography.titleMedium`,
...
|------------------------------------|--------------------------------------|-----------------------------|
| `app_brand`                        | callVault                            | Lowercase. Do not localize. |
...
```
┌──────────────────────────────────────┐
│ ▒ callVault              🔍   ⋮      │  ← NeoTopBar (52.dp + status inset)
...
| `calls_bulk_delete_confirm_body`  | This removes the entries from your CallVault database. Your phone's system call log isn't affected. |
...
unsaved callers that CallVault automatically promoted into a system
contact group using a configurable name pattern (e.g. `callVault-s1 +91…`).
...
CallVault transparently when the row's number matches.
...
data class InquiryRow(
    val normalizedNumber: String,
    val displayLabel: String,           // "callVault-s1 +91…"
    val initial: Char,
    val accentSeed: Int,
    val totalCalls: Int,
...
3. **Group not yet created** — On first auto-save, the contact group is
   ...
   next sync."
4. **User uninstalled CallVault group from system** — `reconcileAutoSaved()`
...
│ ◉ callVault-s1 +91 99887 76655       │
│   1d ago · 1 call     [ Convert ]    │
│ ◉ callVault-s2 +91 90909 90909       │
...
- Row description: "Inquiry, callVault-s1 plus 91 98765 43210, last call 2 hours ago, 4 calls. Double-tap to open. Long-press for bulk select."
- "Convert" button description: "Convert callVault-s1 plus 91 98765 43210 to a real contact, button. Opens Contacts."
...
Provide a clear, scannable index of every CallVault feature that does
