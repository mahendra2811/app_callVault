...
Install + launch CallVault on a connected device.
...
2. Check the APK exists:
   ```bash
   ls -lh "/home/primathon/Documents/p_projet/a_APP/4. callVault/app/build/outputs/apk/debug/app-debug.apk"
   ```
...
4. On confirmation, install:
   ```bash
   adb install -r "/home/primathon/Documents/p_projet/a_APP/4. callVault/app/build/outputs/apk/debug/app-debug.apk"
   ```
...
5. If install fails with `INSTALL_FAILED_UPDATE_INCOMPATIBLE`, ask the user before uninstalling:
   ...
   Then `adb uninstall com.callvault.app` and retry.
