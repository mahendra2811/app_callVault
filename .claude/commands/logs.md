---
description: Tail logcat filtered to callNest + critical Android errors. Runs in the background; you'll see new lines as the app emits them.
---

Tail logs filtered to callNest.
...
- Crash stack traces → flag immediately, suggest `callNest-build-fixer` agent.
- `IllegalStateException: Cannot find ...` → likely a Hilt graph issue.
- WorkManager `Worker result FAILURE` → flag the worker name.
- `SecurityException` in CallLog/Contacts paths → permission issue, suggest user check Settings → Apps → callNest → Permissions.
