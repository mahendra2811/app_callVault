...
- **Lock screen branding** — small "CallVault" wordmark in primary color above the headline.
...
- Worker is wired to `CallVaultApp.onCreate` after onboarding completes.
...
- New notification channel `CHANNEL_HOT_LEAD` (IMPORTANCE_HIGH, vibration on) registered in `CallVaultApp`.
...
- 11 new strings + Hindi translations (pipeline_selected_*, pipeline_bulk_*, bulk_blast_*).
...
- 10 new strings + Hindi translations (pipeline_*).
- Nav route `Destinations.Pipeline` wired into `CallVaultNavHost`.
...
- 13 new strings extracted (templates_*, quickreply_*).
...
- App brand "CallVault" is intentionally untranslated; the welcome tagline and all body strings are localized.
...
### Added
- Settings rows under Privacy: **App lock** (biometric/PIN) and **Help improve CallVault (anonymous analytics)**. Both flags previously only existed in DataStore. (audit v2 #7, follow-up to biometric pass)
...
- Password reset email now passes `redirectUrl = "callvault://auth/recovery"`; `MainActivity` handles the deep link via Supabase + routes to `ResetPasswordScreen`.
...
- Auth nav graph wired into `CallVaultNavHost` — sign-up, forgot, verify, reset routes are reachable. Splash redirects to `AuthDestinations.GRAPH` instead of the old `Destinations.Login`.
...
- `<intent-filter>` for `callvault://auth/*` deep links on `MainActivity`.
