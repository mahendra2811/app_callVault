# Play Store readiness — callNest

**Honest TL;DR up front:** callNest was originally designed as a **sideloaded** app precisely because Google Play's policies make a "call-log inspector" extremely hard to ship. Reading `CallLog.Calls` is a _Restricted Permission_ (`READ_CALL_LOG`). Your app does **not** qualify as a Default Dialer / Caller-ID handler — those are the only two pre-approved use cases. **Realistic outcome: a first submission will almost certainly be rejected** unless you (a) restructure to qualify under one of the exceptions, or (b) replace the call-log capture flow with one that doesn't read the system call log.
...
| Item                | Status                                                  | Action                                                                  |
| ------------------- | ------------------------------------------------------- | ----------------------------------------------------------------------- |
| Application ID      | `com.callNest.app`                                      | Reserve on Play Console; cannot change after first publish.             |
...
Play _will not_ publish without a public-internet privacy policy URL. callNest now collects user data (Supabase auth, FCM tokens, optionally PostHog and Anthropic), so the policy must disclose:
...
- **App category**: _Business_ (most realistic) or _Productivity_
- **Content rating**: complete the IARC questionnaire. callNest should be PEGI 3 / Everyone.
...
callNest is _neither_ — it's a sales-CRM that reads the call log of whatever dialer the user already has. **This use case is not on the approved list.** Apps in this position routinely get rejected with:

> _"Your app violates the Use of SMS or Call Log permission groups policy. Apps that fall outside of the approved use cases must remove all use of these permissions."_
...
| **E. Stay sideloaded.** Don't ship to Play; distribute the APK via your own site / WhatsApp / SMS link. callNest was designed for this.                                                          | Yes — already supported via `versions-stable.json` self-update.    | Zero.                                       |

**My honest recommendation:** Path E (sideloaded) is your _current_ shipping channel. Path B/C is the only Play-friendly one if you must be on Play, and it's a **major** refactor. Path D loses the headline feature.
...
| `app/src/main/java/com/callNest/app/data/work/UpdateCheckWorker.kt`    | Self-updating from `versions.json`                                 | Disable in play flavor                     |
...
Privacy-respecting Android stores that **don't** restrict call-log reading. Listed callNest would reach a niche but real audience without permission-policy headaches. Effort: low.
...
Strip auto-call-log capture entirely; make callNest a manual call-logging + lead-management tool. Still useful, no policy issues. Distinct from your current product — call it **callNest Lite** and keep the full version sideload-only.
