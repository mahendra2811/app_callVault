...
CallVault keeps an encrypted on-device backup of every database table so you can recover from device wipes, factory resets, or a bad app update.
...
## Restore
Tap **Restore from file**, pick a `.cvbk` archive, and enter the passphrase. CallVault confirms with "Replace all data?" before overwriting. There is no undo — the existing database is replaced, not merged.

## Where files live
`Documents/CallVault/backups/`. Move them to your computer, cloud drive, or another device for off-site safety.
