...
class StaleLeadActionReceiver : BroadcastReceiver() {
                ...
                when (action) {
                    ACTION_MARK_CONTACTED -> {
                        if (number.isNotBlank()) {
                            runCatching {
                                noteRepo.upsert(
                                    com.callvault.app.domain.model.Note(
                                        id = 0,
                                        callSystemId = null,
    ...
    companion object {
        const val ACTION_SNOOZE = "com.callvault.app.action.STALE_LEAD_SNOOZE"
        const val ACTION_MARK_CONTACTED = "com.callvault.app.action.STALE_LEAD_MARK_CONTACTED"
