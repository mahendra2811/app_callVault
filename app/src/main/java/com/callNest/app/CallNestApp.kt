 ...
 */
@HiltAndroidApp
class CallNestApp : Application(), Configuration.Provider {
    ...
    @Inject lateinit var demoSeeder: com.callNest.app.data.demo.DemoSeeder
            ...
            try {
                if (settingsDataStore.onboardingComplete.first()) {
                    ...
                    if (settingsDataStore.followUpRemindersEnabled.first()) {
                        runCatching { StaleLeadNudgeWorker.schedule(this@CallNestApp) }
                            .onFailure { Timber.w(it, "StaleLeadNudgeWorker.schedule failed") }
                    }
                    if (settingsDataStore.weeklyDigestEnabled.first()) {
                        runCatching {
                            com.callNest.app.data.work.WeeklyDigestWorker.schedule(this@CallNestApp)
